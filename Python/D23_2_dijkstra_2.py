...
def main(DATA_INPUT):
    ...
    for i, line in enumerate(my_map):
        for j, char in enumerate(line):
            if char in '^v<>':
                my_map[i] = my_map[i][:j] + '.' + my_map[i][j + 1:]
    ...
    Q = [(0, 0)]
    while Q:
        dis_this_node, this_node = heapq.heappop(Q)
        dis_this_node *= -1
        for j, dis in enumerate(network[this_node]):
            new_dis = dis + dis_this_node
            if nodes[j][0] < new_dis:
                nodes[j] = [new_dis, this_node]
                heapq.heappush(Q, (-new_dis, j))
